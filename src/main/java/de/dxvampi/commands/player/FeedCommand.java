...
public class FeedCommand implements CommandExecutor, TabCompleter {
    ...
    public FeedCommand(DeEssentials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NonNull CommandSender sender, @NonNull Command command, @NonNull String label, String @NonNull [] args) {
        ...
        // Base permission
        if (!sender.hasPermission("deessentials.feed")) {
        ...
        if (args.length == 0) {
            ...
            player.sendMessage(MessageUtils.getColoredMessage("&aYou have been fed!"));
            return true;
        }
        ...
        if (target != sender) {
            if (!sender.hasPermission("deessentials.feed.others")) {
                CommandErrors.RaiseInsufficientPermission(plugin, sender, label, args, "You can only feed yourself");
                return true;
            }
        }
        ...
        target.sendMessage(MessageUtils.getColoredMessage("&aYou have been fed!"));

        // Does not send the message twice
        if (target != sender) {
            sender.sendMessage(MessageUtils.getColoredMessage("&aYou have successfully fed &f" + target.getDisplayName() + "&a!"));
        }
